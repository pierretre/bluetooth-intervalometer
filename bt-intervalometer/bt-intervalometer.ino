...
void loop() {
  ...
  if (inputString == "STATUS") {
  ...
  } else if (inputString.substring(4, 10) == "RUN") {
    ...
    if (shutter_speed <= 0 || shutter_speed > 35) {
      ...
      mySerial.write("STATUS|WAITING|You must specify a shutter speed between 0.001 and 35 \n");
      OK_values = false;
    }
    if (time_between_pics <= ((int) 1.5 * shutter_speed)) {
      ...
      mySerial.write("STATUS|WAITING|You must specify more time between pictures for your camera to process safely \n");
      OK_values = false;
    }
    if (pics_number <= 0) {
      //error message to tell him that he can't take 0 or less pictures
      mySerial.write("STATUS|WAITING|You must take at least on picture \n");
      OK_values = false;
    }
  }
  if (OK_values) {
